...
{
    public class DebugTeamFighter : Action

    {
        ...
        {
            if (Plugin.FChatClient.IsUserAdmin(character, channel) && (Plugin.CurrentTeamBattlefield.IsActive))
            {
                if (args.Count() < 3)
                {
                    return;
                ...
                var activeFighter = Plugin.CurrentTeamBattlefield.GetFighter(characterName);
